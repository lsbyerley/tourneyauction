...
export default async (req, res) => {
  ...
  try {
    const { updatedAuction } = await graphCMSMutationClient.request(UpdateAuctionEndDateQuery, {
      data: req.body,
    });

    const { publishedAuction } = await graphCMSMutationClient.request(PublishAuctionQuery, {
